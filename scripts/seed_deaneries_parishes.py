...
def seed_data():
    ...
    try:
        ...
        for zone_name, deaneries_dict in seed_data.items():

            # Handle Zone
            zone_obj = db.query(Zone).filter_by(name=zone_name.strip()).first()
            if not zone_obj:
                zone_obj = Zone(name=zone_name.strip())
                db.add(zone_obj)
                db.flush()  # get ID
                print(f"Created Zone: {zone_name}")

            # --- 2. Loop through Deaneries ---
            for deanery_name, parishes_dict in deaneries_dict.items():
                deanery_obj = db.query(Deanery).filter_by(name=deanery_name.strip(), zone_id=zone_obj.id).first()
                if not deanery_obj:
                    deanery_obj = Deanery(name=deanery_name.strip(), zone_id=zone_obj.id)
                    db.add(deanery_obj)
                    db.flush()
                    print(f"  → Created Deanery: {deanery_name} (Zone: {zone_name})")

                # --- 3. Loop through Parishes ---
                for parish_name, outstations_list in parishes_dict.items():
                    ...
                    # --- 4. Loop through Outstations ---
                    for outstation_name in outstations_list:
                        outstation_name = outstation_name.strip()
                        if not outstation_name:
                            continue  # skip blank names
                        outstation_obj = db.query(Outstation).filter_by(
                            name=outstation_name,
                            parish_id=parish_obj.id
                        ).first()
                        if not outstation_obj:
                            db.add(Outstation(name=outstation_name, parish_id=parish_obj.id))
